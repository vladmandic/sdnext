...
class DAAMScript(scripts_manager.Script):
    ...
    def run(self, p: processing.StableDiffusionProcessing, append_images, colormap): # pylint: disable=arguments-differ
        ...
        if shared.sd_model_type != 'sdxl':
            log.warning(f'DAAM: pipeline={c} required=StableDiffusionXLPipeline')
            return None
        ...
        with daam.trace(shared.sd_model) as tc:
            ...
            log.info(f'DAAM: prompt="{global_heat_map.prompt}" heatmaps={global_heat_map.heat_maps.shape}')
            ...
            if append_images:
                ...
                for parsed_heat_map in parsed_heat_maps:
                    if len(parsed_heat_map.token.text) > 1:
                        log.debug(f'DAAM: token="{parsed_heat_map.token.text}"')
