    ...
    def run(self, p: processing.StableDiffusionProcessing, model, weight): # pylint: disable=arguments-differ
        ...
        if shared.sd_model_type == 'sd':
            if not model.startswith('SD15'):
                log.warning(f'ResAdapter: pipeline={shared.sd_model_type} selected={model}')
                return None
        ...
        log.debug(f'ResAdapter: pipeline={shared.sd_model.__class__.__name__} model="{model}" weight={weight} file="{models[model]}"')
