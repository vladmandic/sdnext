...
export function showCardDetails(event) {
...
function getCardDetails(...args) {
  ...
  const el = (event?.target as any)?.parentElement?.parentElement;
...
function getCardsForActivePage() {
  ...
  if (!pageName) return [];
  let allCards = Array.from<any>(gradioApp().querySelectorAll('.extra-network-cards > .card'));
...
async function filterExtraNetworksForTab(searchTerm) {
  ...
  const pagename = getENActivePage();
  if (!pagename) return;
  const allPages = Array.from<any>(gradioApp().querySelectorAll('.extra-network-cards'));
  const pages = allPages.filter((el) => el.id.toLowerCase().includes(pagename.toLowerCase()));
  for (const pg of pages) {
    const cards = Array.from<any>(pg.querySelectorAll('.card') || []);
    items += cards.length;
...
export function applyStyles(styles?: any) {
  ...
  if (styles) {
    ...
    newStyles = Array.from<any>(styles).map((el) => el.textContent).filter((el) => el.length > 0);
  }
...
export function quickApplyStyle() {
  const tabName = getENActiveTab();
  const btnApply = gradioApp().getElementById(`${tabName}_extra_apply`);
  if (btnApply) btnApply.click();
}

export function quickSaveStyle() {
  const tabName = getENActiveTab();
...
function setupExtraNetworksForTab(tabName) {
  ...
  const resizeObserver = new ResizeObserver((entries) => {
    for (const entry of entries) {
      for (const el of Array.from<any>(gradioApp().getElementById(`${tabName}_extra_tabs`).querySelectorAll('.extra-networks-page'))) {
  ...
  const intersectionObserver = new IntersectionObserver((entries) => {
    ...
    const cards = Array.from<any>(gradioApp().querySelectorAll('.extra-network-cards > .card'));
