...
from .transformer_boogu import BooguImageTransformer2DModel, get_freqs_cis
...
class BooguImagePipeline(DiffusionPipeline):
    ...
    def preprocess_vlm_input_pil_images(
        ...
        crops_coords: List[Tuple[int, int, int, int]] | None = None,
    ...
    def prepare_image(
        ...
        _success, _max_images_per_sample, wrapped_input_images = self._check_and_wrap_input_images(images)
        ...
        for _i, img in enumerate(wrapped_input_images):
    ...
    def encode_prompt(
        ...
        batch_size, _seq_len, _ = instruction_embeds.shape

        batch_size, _seq_len, instruction_embeds, instruction_attention_mask = self._reshape_embeds_and_mask(
            instruction_embeds,
    ...
    def __call__(
        ...
        # SDN: standardize args
        instruction = prompt
        negative_instruction = negative_prompt
        ...
        if input_images:
            _success, max_images_per_sample, input_images = self._check_and_wrap_input_images(input_images)
        ...
        input_images, width, height, _ori_width, _ori_height = self._resolve_output_and_original_size(
            input_images=input_images,
            ...
            for i, t in enumerate(timesteps):
                ...
                if (task_type == "ti2i") and (text_guidance_scale > 1.0) and (image_guidance_scale > 1.0):  # Checked
                    ...
                    if use_empty_neg_instruct_4_ref_img_pred_at_image_guide_in_double_guide:
                        model_pred_drop_text_pos = model_pred_drop_text_empty_instruct # pylint: disable=possibly-used-before-assignment
