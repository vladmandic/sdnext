...
class BooguImageDoubleStreamRotaryPosEmbed(nn.Module):
    ...
    def forward(
        ...
        for i, (cap_seq_len, ref_img_len, img_len, _seq_len) in enumerate(
            zip(
                l_effective_cap_len,
                l_effective_ref_img_len,
                l_effective_img_len,
...
class BooguImageDoubleStreamSelfAttnProcessor(nn.Module):
    ...
    def __call__(
        ...
        encoder_seq_lengths: List[int] | None = None,  # [B] - Instruction sequence lengths for each sample
        seq_lengths: List[int] | None = None,  # [B] - Total sequence lengths for each sample
    ) -> torch.Tensor:
        """
