...
def restore_state(p: processing.StableDiffusionProcessing):
    if p.state in ['reprocess_refine', 'reprocess_detail']:
        # validate
        if last_p is None:
            log.warning(f'Restore state: op={p.state} last state missing')
            return p
        if p.__class__ != last_p.__class__:
            log.warning(f'Restore state: op={p.state} last state is different type')
            return p
        if shared.history.count == 0:
            log.warning(f'Restore state: op={p.state} last latents missing')
            return p
        state = p.state
        ...
        log.info(f'Restore state: op={p.state} skip={p.skip}')
    return p
...
def process_hires(p: processing.StableDiffusionProcessing, output):
    ...
    if p.enable_hr:
        ...
        if (getattr(shared.sd_model, 'controlnet', None) is not None) and (((isinstance(shared.sd_model.controlnet, list) and len(shared.sd_model.controlnet) > 1)) or ('Multi' in type(shared.sd_model.controlnet).__name__)):
            log.warning(f'Process: control={type(shared.sd_model.controlnet)} not supported in hires')
            return output
        ...
        if p.hr_force and strength == 0:
            log.warning('Hires skip: denoising=0')
            p.hr_force = False
        if p.hr_force:
            ...
            try:
                ...
                shared.state.interrupted = True
                log.error(f'Processing step=hires: args={hires_args} {e}')
                errors.display(e, 'Processing')
                modelstats.analyze()
            finally:
                process_post(p)
    ...
    if is_refiner_enabled(p):
        ...
        for i in range(len(output.images)):
            ...
            try:
                ...
                shared.state.interrupted = True
                log.error(f'Processing step=refine: args={refiner_args} {e}')
                errors.display(e, 'Processing')
                modelstats.analyze()
...
def process_diffusers(p: processing.StableDiffusionProcessing):
    results = []
    if debug:
        log.trace(f'Process diffusers args: {vars(p)}')
