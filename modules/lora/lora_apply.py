            ...
            if fuse:
                ...
                if hasattr(self, "sdnq_dequantizer"):
                    ...
                    if getattr(self, "codebook", None) is not None:
                        self.sdnq_codebook_backup = self.codebook.clone().to(devices.cpu)
                    else:
                        self.sdnq_codebook_backup = None
        ...
        try:
            ...
            if hasattr(self, "sdnq_dequantizer_backup"):
                weight = self.sdnq_dequantizer_backup(
                    ...
                    codebook=self.sdnq_codebook_backup.to(devices.device) if getattr(self, "sdnq_codebook_backup", None) is not None else None,
                ...
                weight = self.sdnq_dequantizer(
                    ...
                    codebook=self.codebook.to(devices.device) if getattr(self, "codebook", None) is not None else None,
...
def network_add_weights(self: torch.nn.Conv2d | torch.nn.Linear | torch.nn.GroupNorm | torch.nn.LayerNorm | diffusers.models.lora.LoRACompatibleLinear | diffusers.models.lora.LoRACompatibleConv, model_weights: torch.Tensor | None = None, lora_weights: torch.Tensor = None, deactivate: bool = False, device: torch.device = None, bias: bool = False):
    ...
    if not bias and hasattr(self, "sdnq_dequantizer"):
        try:
            ...
            self.weight = torch.nn.Parameter(new_weight, requires_grad=False)
            self.sdnq_dequantizer = self.scale = self.zero_point = self.svd_up = self.svd_down = self.codebook = None
    ...
    if weights_backup is not None and not isinstance(weights_backup, bool):
        ...
        if updown is not None:
            ...
            if hasattr(self, "sdnq_dequantizer_backup"):
                ...
                if getattr(self, "sdnq_codebook_backup", None) is not None:
                    self.codebook = torch.nn.Parameter(self.sdnq_codebook_backup.to(device), requires_grad=False)
                else:
                    self.codebook = None
