...
def add_http_args(p):
    ...
    p.add_argument("--api-rate-limit", default=env_flag("SD_APIRATELIMIT", False), action='store_true', help="Enable per-IP rate limiting on generation API endpoints, default: %(default)s")
...
def add_diag_args(p):
    ...
    p.add_argument('--enso', default=env_flag("SD_ENSO", False), action='store_true', help="Build Enso frontend on startup if sources changed, default: %(default)s")
