    ...
    if not force:
        try:
            if (checkpoint_info.path.endswith('.safetensors') and os.path.isfile(checkpoint_info.path)): # skip check for single-file safetensors models
                return True
            if os.path.exists(checkpoint_info.path) and os.path.isdir(checkpoint_info.path) and any(os.path.isfile(os.path.join(checkpoint_info.path, f)) for f in ('model_index.json', 'modular_model_index.json', 'config.json')): # skip check for local model folders
                return True
        except Exception:
            pass
    repo_id = path_to_repo(checkpoint_info)
