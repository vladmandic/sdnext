...
export function useModelAnalysis(enabled = false) {
  return useQuery({
    ...
    queryFn: () => api.get<ModelAnalysis>("/sdapi/v2/model/analyze"),
    enabled,
    staleTime: 0,
  });
}
...
export function useModelListDetail() {
  return useQuery({
    ...
    queryFn: () => api.get<ModelListDetail[]>("/sdapi/v2/model/list-detail"),
    staleTime: 60_000,
  });
}
...
export function useHfSearch(keyword: string, enabled = false) {
  return useQuery({
    ...
    queryFn: () => api.get<HfModelResult[]>("/sdapi/v2/model/hf/search", { keyword }),
...
export function useMergeMethods() {
  return useQuery({
    ...
    queryFn: () => api.get<MergeMethodsInfo>("/sdapi/v2/model/merge/methods"),
    staleTime: Infinity,
  });
}
...
export function useMergeModels() {
  return useMutation({
    mutationFn: (req: MergeRequest) => api.post<{ status: string }>("/sdapi/v2/model/merge", req),
  });
}
...
export function useLoaderPipelines() {
  return useQuery({
    ...
    queryFn: () => api.get<{ pipelines: string[] }>("/sdapi/v2/model/loader/pipelines"),
    staleTime: Infinity,
  });
}
...
export function useLoraLoaded(enabled = true) {
  return useQuery({
    ...
    queryFn: () => api.get<{ loras: string[] }>("/sdapi/v2/model/lora/loaded"),
    enabled,
    staleTime: 30_000,
  });
}
