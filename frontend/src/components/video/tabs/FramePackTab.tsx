  ...
  return (
    <div className="space-y-1">
      <ParamSection title="Model">
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            <Label className="text-[11px] text-muted-foreground w-16 shrink-0">Variant</Label>
            <Combobox value={fpVariant} onValueChange={(v) => setParam("fpVariant", v)} options={variants ?? ["bi-directional", "forward-only"]} className="h-7 text-xs flex-1" />
          </div>
      ...
      <ParamSection title="Model Options" defaultOpen={false}>
        ...
        />
        <div className="flex items-center gap-2">
          <Label className="text-[11px] text-muted-foreground w-16 shrink-0">TeaCache</Label>
          <Switch checked={fpTeacache} onCheckedChange={(v) => setParam("fpTeacache", v)} />
        </div>
        <div className="flex items-center gap-2">
          <Label className="text-[11px] text-muted-foreground w-16 shrink-0">Optimized</Label>
          <Switch checked={fpOptimizedPrompt} onCheckedChange={(v) => setParam("fpOptimizedPrompt", v)} />
        </div>
        <div className="flex items-center gap-2">
          <Label className="text-[11px] text-muted-foreground w-16 shrink-0">CFG Zero</Label>
          <Switch checked={fpCfgZero} onCheckedChange={(v) => setParam("fpCfgZero", v)} />
        </div>
        <div className="flex items-center gap-2">
          <Label className="text-[11px] text-muted-foreground w-16 shrink-0">Preview</Label>
          <Switch checked={fpPreview} onCheckedChange={(v) => setParam("fpPreview", v)} />
        </div>
        <div className="flex items-center gap-2">
          <Label className="text-[11px] text-muted-foreground w-16 shrink-0">Attention</Label>
          <Combobox value={fpAttention} onValueChange={(v) => setParam("fpAttention", v)} options={["Default", "sdpa", "flash", "sage", "xformers"]} className="h-7 text-xs flex-1" />
        </div>
        <div className="flex items-center gap-2">
          <Label className="text-[11px] text-muted-foreground w-16 shrink-0">VAE</Label>
          <Combobox value={fpVaeType} onValueChange={(v) => setParam("fpVaeType", v)} options={["Full", "Tiny"]} className="h-7 text-xs flex-1" />
        </div>
      </ParamSection>
