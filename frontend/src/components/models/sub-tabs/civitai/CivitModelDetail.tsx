...
import { Check, ChevronDown, ChevronRight, Download, Loader2, Bookmark, Ban, MessageSquareText } from "lucide-react";
  ...
  return (
    <div className="border border-border/50 rounded-md">
      <button type="button" onClick={() => setOpen(!open)} className="flex items-center gap-2 w-full px-3 py-2 text-left hover:bg-muted/30">
        ...
        <span className="text-xs font-medium truncate min-w-0">{version.name}</span>
        <Badge variant="outline" className="text-4xs px-1 py-0 shrink-0 ml-auto">{version.baseModel}</Badge>
      </button>
      {open && (
        <div className="px-3 pb-3 space-y-2">
          {/* Trigger words — collapsible, capped height */}
          {version.trainedWords.length > 0 && (
            <div>
              <button
                type="button"
                onClick={() => setShowTriggers(!showTriggers)}
                className="flex items-center gap-1.5 text-3xs text-muted-foreground hover:text-foreground transition-colors"
              >
                <MessageSquareText className="h-3 w-3 shrink-0" />
                <span>Trigger words ({version.trainedWords.length})</span>
                {showTriggers ? <ChevronDown className="h-2.5 w-2.5" /> : <ChevronRight className="h-2.5 w-2.5" />}
              </button>
              {showTriggers && (
                <div className="mt-1 max-h-20 overflow-y-auto rounded bg-muted/30 p-2">
                  <div className="flex flex-wrap gap-1">
                    {version.trainedWords.map((w) => (
                      <Badge key={w} variant="secondary" className="text-4xs px-1 py-0 max-w-full">
          ...
          {/* Files — always visible, download buttons pinned */}
          ...
          <>
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-sm min-w-0">
                <span className="truncate min-w-0">{model.name}</span>
            ...
            {model.tags.length > 0 && (
              <div className="flex gap-1 overflow-hidden">
                {model.tags.slice(0, 12).map((t) => (
                  <Badge key={t} variant="secondary" className="text-4xs px-1 py-0 shrink-0">{t}</Badge>
                ))}
              </div>
            )}
