...
export function ExtractLoraSubTab() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-[11px] text-muted-foreground">Extract LoRA from loaded model differences.</p>
      ...
      <div className="space-y-1">
        ...
        {loras.length > 0 ? (
          <div className="space-y-0.5">
            {loras.map((name) => (
              <p key={name} className="text-[11px] font-mono px-2 py-0.5 bg-muted/30 rounded">{name}</p>
            ))}
          </div>
        ) : (
          <p className="text-[11px] text-muted-foreground">No LoRAs currently loaded in model.</p>
        )}
      </div>

      <div>
        <Label className="text-[11px]">Output filename</Label>
        <Input className="h-7 text-xs" value={filename} onChange={(e) => setFilename(e.target.value)} placeholder="extracted-lora" />
      </div>
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="extract-te" className="text-[11px]">Text Encoder</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox id="extract-unet" checked={includeUnet} onCheckedChange={(v) => setIncludeUnet(!!v)} />
            <Label htmlFor="extract-unet" className="text-[11px]">UNet</Label>
          </div>
        </div>
      </div>

      <div>
        <Label className="text-[11px]">Max rank: {maxRank}</Label>
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <Label htmlFor="extract-autorank" className="text-[11px]">Auto rank</Label>
        </div>
        {autoRank && (
          <div className="pl-5">
            <Label className="text-[11px]">Rank ratio: {rankRatio.toFixed(2)}</Label>
      ...
      <div className="flex items-center gap-2">
        ...
        <Label htmlFor="extract-overwrite" className="text-[11px]">Overwrite existing</Label>
      </div>
      ...
      {extract.data && <p className="text-[11px] text-muted-foreground">{extract.data.status}</p>}
    </div>
  );
}
